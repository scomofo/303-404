...
- **Song Bank** (Week 8) — a gallery of loadable step patterns (notes, accents, slides, waveform, tempo). Tap a card to load it into its own independent TD-3 engine (its own oscillator/filter/gain chain, not the Week 2/6 one) and hit Play to hear it. 14 cards ship by default:
  ...
  - **Pattern chart** — loading a card also renders it back into the chart layout the transcriptions came from: pitch class on the NOTE row, octave as a D/U mark against the pattern's home octave (the octave most of its steps sit in, since a chart records a shift away from where a pattern lives rather than an absolute pitch), then the accent/slide row, waveform and filter knobs. Anything no source stated — pattern group, bank, knob positions — shows a dash, and an unknown knob draws no pointer rather than a default one. Env Mod is blank on every card because this engine models filter decay but not modulation depth.
