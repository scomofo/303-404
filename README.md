...
| 9 | Drum Bank — Rhythm Pattern Sheets | A bank of found drum patterns, each drawn as a TR-808 pattern sheet and playable at its own tempo |

A progress bar tracks your position across the 33 steps, and the **Course map** button in the header jumps to any step.
...
- **Drum Bank** (Week 9) — a bank of drum patterns found in other people's published work, each drawn as a **TR-808 rhythm pattern sheet**: AC on top, then BD, SD, LT, MT, HT, RS, CP, CB, CY, OH and CH, across 16 or 32 steps — the two lengths 808 sheets are printed in — with the four downbeats of every bar tinted and a voice the pattern never plays left greyed and empty. Hit a card's **Load into engine** to hand it to an independent drum engine (synthesized 808-ish voices, not samples: pitched sines with a fast drop for kick and toms, filtered noise for everything with a hiss in it, two detuned squares for the cowbell, four staggered bursts for the clap), then **Play Pattern** to loop it at that card's own tempo. 11 pattern sheets ship by default:
  - **Three transcriptions made on the 808 itself**, read out of the [`tr808r`](https://github.com/coolbutuseless/tr808r) package (MIT) rather than re-typed: "Sexual Healing" (Marvin Gaye), "Beat It" (Michael Jackson — the intro) and "Confusion" (New Order). Each of those demo songs is a chain of 16-step patterns; a card carries the first two of the chain, so its sheet is 32 steps, and its notes say how many more patterns the source chains after them. The tempo shown is the one the source states.
  - **Seven patterns from the [GiantSteps `drum-pattern-datasets`](https://github.com/GiantSteps/drum-pattern-datasets) collection** — the breaks "Funky Drummer", "Impeach the President" and "When the Levee Breaks", plus the standard "Electro" beat and the "House", "Classic House" and "Minimal Techno" genre templates its `EDM-literature-patterns` file collects from EDM production literature. Those entries are written in General MIDI drum notes rather than for any one machine, so the mapping onto 808 rows is this guide's, and each card names the dataset entry it came from. None of them carries a tempo, so all seven play at one shown as `~X BPM (estimated)`.
  - **One pattern this course wrote itself** — the Week 1 four-on-the-floor. It is the only card with an accent row, a pattern number and a variation, because it is the only one whose source (this guide) records them.
  - **Nothing a source left out is filled in.** No outside source here writes an accent row, names a pattern number, variation or pre-scale, or states where on a machine its pattern was written, so those rows and boxes print blank or dashed on every card but the practice one. The tests enforce this rather than trusting it: adding an accent row to a transcribed card fails the suite until its source changes.
  - **The sheet is a reading format, not a provenance claim.** All three source types render in the same layout, and neither the app nor this file claims that any of these patterns was printed on a Roland sheet. The two outside sources were picked because their pattern data is published in a form that can be read and re-derived — an MIT-licensed R package and a public dataset — rather than squinted at in a photograph; every row array in `DRUM_CARDS` was generated from them rather than typed by hand.
  - **Playing them elsewhere.** In an 808 emulator the rows map one-for-one. On an SP-404, assign one pad per row and enter the same steps in TR-REC. On the RD-6, BD, SD, CH and OH and the accent row exist as-is; the toms, cowbell, rimshot, clap and cymbal do not, so play them on whatever voice you can spare or leave them out.

  Add your own by appending to the `DRUM_CARDS` array in the `Component` class. Each entry is `{ id, title, artist?, sourceType, steps, bpm, bpmConfirmed?, patternNo?, variation?, preScale?, pair?, voicing, tag, source, rows }`. `sourceType` is `record`, `literature` or `practice`; `steps` is 16 or 32; `rows` holds one array of 0-based step indices per voice (`ac`, `bd`, `sd`, `lt`, `mt`, `ht`, `rs`, `cp`, `cb`, `cy`, `oh`, `ch`), and a voice the pattern never plays is left out rather than given an empty array. Omit `bpmConfirmed` and the tempo renders as an estimate. `voicing` says whether the source was written for this machine or mapped onto it, and `source` is the citation printed on the sheet. RS and CP are switch pairs on the 808 panel, so `pair` names the side a card means — `{ rs: 'CL' }` for claves, `{ cp: 'MA' }` for maracas — and both the printed row and the voice that sounds follow it.
...
| `test/drumbank.test.mjs` | Every drum card sits on the 808 row set at a printed sheet length; each sheet shows exactly what its card hands the engine; a source that recorded no accent row, pattern number, variation or pre-scale still has none; an unstated tempo still renders as an estimate; the RS/CL and CP/MA switch pairs print and sound the side a card names; accent raises a step's level without adding a voice; loading a card carries its own tempo and length; leaving the step stops the engine |
| `test/timing.test.mjs` | Notes are scheduled ahead of the audio clock and land on an exact grid; Stop silences queued hits; shuffle swings without changing bar length or leaking cleanup timers; TD-3/Song Bank filter envelopes survive lookahead scheduling; a Drum Bank card runs on its own tempo grid rather than the RD-6's fixed one; DDJ phase jumps stay on-grid |
